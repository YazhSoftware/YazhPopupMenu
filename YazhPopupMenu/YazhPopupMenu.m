...
#import "YazhPopupMenu.h"

NSString * const YazhPopupMenuButtonClickedNotification = @"YazhPopupMenuButtonClickedNotification";

@interface YazhPopupMenu ()
...
@implementation YazhPopupMenu

#pragma mark - LifeCycle
+ (instancetype)singleton
{
    static dispatch_once_t onceToken;
    static YazhPopupMenu *_singleton;
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event
{
    ...
    if(!_menuOpened)
    {
        // return nil 代表可以穿透 YazhPopupMenu 操作下方 UIViewController
        return nil;
    }
...
#pragma mark Action for tapping in YazhPopMenu self
    ...
    if(!animated)
    {
        ...
        if(button)
        {
            [[NSNotificationCenter defaultCenter]
             postNotificationName:YazhPopupMenuButtonClickedNotification object:@(button.tag)];
        }
...
- (void)__menuWillOpen
{
    if(![self conformsToProtocol:@protocol(YazhPopupMenuProtocol)] ||
